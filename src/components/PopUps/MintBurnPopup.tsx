...
export const MintBurnPopup: React.FC<Props> = ({ info, text, isMint, onClose, art, isSpinner, setBurnArtFailed, setBurnArtSuccess }) => {
    ...
    const handleConfirm = async () => {
        if (isMint) {
            ...
            if (isRaffleArt(art)) {
                await mintImage({ title: "Art Battle", mediaUrl: art?.colouredArt, artId: art?._id, queryType: queryType, referenceUrl: art?.colouredArtReference, count: art?.raffleCount, contractId: contractId })
            }
        } else {
    ...
    return (
        <div className="sign-in-popup fixed top-0 bottom-0 left-0 right-0 z-50 h-full flex items-center justify-center px-3">
            <div className="signin-card w-[25rem] h-auto lg:p-8 md:p-6 p-4 rounded-2xl bg-black">
                {!burningArt && <div className="close-art w-full flex justify-end">
                ...
                {!burningArt && <div className="popup-btns w-full flex justify-center items-center gap-3 py-1 mb-5">
