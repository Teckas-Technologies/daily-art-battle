...
const ArtPiece: React.FC<ArtPieceProps> = ({ art, onVote, success,votedFor }) => {
    ...
    width={100}
    height={100}
 className=" w-36 h-36 md:shrink-0 max-w-full max-h-full lg:w-80 lg:h-80"
 
  />

 
  <p className="mt-2 text-black py-2 text-xs sm:text-sm font-small break-words text-center sm:break-all md:break-normal">{art.title} by {art.artistId}</p>
            {votedFor == art.name ?(
