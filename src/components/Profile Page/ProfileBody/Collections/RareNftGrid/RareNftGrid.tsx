...
import { useFetchUserRareNfts } from "@/hooks/userNftsHook";
import { ConfirmPopupInfo, NftToken } from "@/types/types";
import { useSearchUserRareNfts } from "@/hooks/searchUserNfts";
...
export const RareNftGrid: React.FC<Props> = ({ rendered, setConfirmPopup }) => {
    const [page, setPage] = useState<number>(1);
    const [sort, setSort] = useState<string>("dateDsc");
    const { arts, loading, totalPage, fetchUserRareNfts } = useFetchUserRareNfts();
    const { searchLoading, searchedArts, totalSearchPage, searchUserRareNfts } = useSearchUserRareNfts();
    ...
    useEffect(() => {
        const initializeData = async () => {
            const limit = 8;
            if (!searchQuery) {
                fetchUserRareNfts(sort, page, limit);
            } else {
                searchUserRareNfts(searchQuery, page, limit);
            }
        };
        if (userDetails) {
    ...
    const handleNext = () => {
        if (page < total) {
            ...
            const limit = 8;
            fetchUserRareNfts(sort, page + 1, limit);
    ...
    const handlePageClick = (pageNumber: number) => {
        if (pageNumber !== page) {
            setPage(pageNumber);
            const limit = 8;
            fetchUserRareNfts(sort, pageNumber, limit);
