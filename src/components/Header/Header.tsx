...
const navs = [
  {
    ...
    label: "Battles",
    path: "/",
    icon: "/images/Battle_New.png",
  },
  {
    id: "leaderboard",
    label: "Leaderboard",
    path: "/leaderboard",
    icon: "/images/Leaderboard_New.png",
  },
  {
    id: "campaigns",
    label: "Campaigns",
    path: "/campaign",
    icon: "/images/Campaign_New.png",
  },
  ...
  return (
    <div className="header fixed bg-black h-[7rem] w-full z-50 top-0 left-0 flex items-center justify-between xl:px-[7rem] lg:px-[3rem] md:px-[2rem] px-3">
      <div className="header-left flex items-center justify-center px-4 gap-5 rounded-[7rem] md:px-10 py-2">
        <div className="nav-links-logo flex items-center gap-2">
          <Link href={"/"} className="flex items-center gap-2">
            ...
            <h2 className="text-white hidden md:block spartan-bold font-bold text-md">
              GFXvs
            </h2>
          </Link>
            ...
            <div key={nav?.id}>
              {nav.id === "create" ? (
                <>
                  <h3
                    key={index}
                    className={`flex items-center gap-1 text-white cursor-pointer font-medium spartan-medium text-sm`}
                    onClick={() => {
                      if (!userDetails) {
                        setSignToast(true);
                ...
                <>
                  <h3
                    key={index}
                    className={`relative flex items-center gap-1 text-white cursor-pointer font-medium spartan-medium text-sm ${
                      pathName === nav.path || pathName === "/previous"
                  ...
                  >
                    ...
                    {subNav && (
                      <div
                        ref={subMenuRef}
                        className="sub-navs absolute top-[200%] flex flex-col gap-3 left-[-10px] w-[250%] p-3 rounded-[0.75rem]"
                      >
                        {subNavs.map((nav, index) => {
                          const isActive =
                            (nav.id === "upcomingbattles" &&
                          ...
                          return (
                            <div
                              key={index}
                              className={`sub-nav flex justify-start items-center gap-2 ${
                                isActive ? "active" : ""
                              }`}
                              onClick={() => {
                                // setSubNav(false);
                  ...
                  >
                    ...
                    <div
                      className={`md:hidden lg:block ${
                        nav.id === "battles" || nav?.id === "campaigns"
                          ? "w-[1.3rem] h-[1.3rem]"
                          : "w-[1.3rem] h-[1.3rem]"
                      }`}
                    >
      ...
      <div className="header-right flex items-center md:gap-3 gap-2">
        ...
        {userDetails && (
          <div
            className="outside relative rounded-xl cursor-pointer"
          ...
          >
            <div className="layer2 rounded-xl">
              <div className="header-info flex items-center gap-3 px-5 py-2 rounded-xl">
                ...
                <div className="name-id hidden xl:block lg:hidden md:max-w-[8rem] lg:max-w-[10rem] xl:max-w-[12rem]">
                  <h2 className="spartan-bold font-bold text-md text-center truncate">
                    {userDetails?.user?.firstName +
                      " " +
                      userDetails?.user?.lastName}
                  </h2>
                  <h4 className="spartan-light text-sm text-center email truncate">
                    {userDetails?.user?.email}
                  </h4>
                </div>
                <div className="name-id hidden xl:hidden lg:block md:hidden hidden">
                  <h2 className="spartan-bold font-bold text-lg text-center">
                ...
                <div className="gfx-points flex lg:flex-col gap-1">
                  <div className="point-name flex items-center gap-1">
                    <InlineSVG src="/icons/gfx-point.svg" className="h-5 w-5" />
                    <h2 className="spartan-semibold hidden lg:block">GFXvs</h2>
                  </div>
                  <div className="points">
                    <h2 className="md:spartan-bold spartan-semibold text-center font-bold">
                      {userDetails?.user?.gfxCoin?.toFixed(1)}
                    </h2>
                  </div>
                </div>
              </div>
            ...
            {openProfileMenu && (
              <div
                ref={profileMenuRef}
                className="profile-dd absolute top-[100%] right-0 xl:w-full w-[125%] h-auto rounded-[0.75rem] md:p-6 p-4"
              >
                <div className="profile-menus flex flex-col gap-5">
                  ...
                  <Link href={"/profile?walletHistory=true"}>
                    <div className="wallet-history flex items-center justify-start gap-2 cursor-pointer">
                      <InlineSVG src="/icons/swap.svg" className="h-5 w-5" />
                      <h2>Transaction History</h2>
                    </div>
                  </Link>
                </div>
                <div
                  className="signout-btn flex items-center justify-center gap-2 rounded-md py-2 mt-5 cursor-pointer"
                  onClick={signOutUser}
                >
                  <InlineSVG src="/icons/signout.svg" className="h-5 w-5" />
