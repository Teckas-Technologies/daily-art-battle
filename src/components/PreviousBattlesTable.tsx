    ...
    <table className="min-w-full mt-4 overflow-hidden ">
        <thead>
            <tr className="text-xs md:text-2xl sm:text-xl">
                <th className="text-center  font-normal text-black" style={{ borderTopLeftRadius: 5, borderBottomLeftRadius: 5 }}>UNIQUE RARE</th>
                <th className="text-center  font-normal text-black">RARE OWNER</th>
                <th onClick={() => handleSort('vote')} className="text-black text-center cursor-pointer font-normal hover:underline">
                    <span className="flex items-center justify-center hover:underline">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1.25em" height="1em" viewBox="0 0 640 512" className="ml-1">
                ...
                <th onClick={() => handleSort('date')} className="text-black text-center cursor-pointer font-normal hover:underline">
                    <span className="flex items-center justify-center ">
                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" className="ml-1">
                            <g fill="none">
        ...
        <a href={`https://www.tradeport.xyz/near/collection/${SPECIAL_WINNER_CONTRACT}?bottomTab=trades&tab=items&tokenId=${battle.tokenId}`} className="flex flex-col items-center h-full w-full">
            <div className="relative w-full h-full" style={{ borderTopLeftRadius: 40, borderBottomLeftRadius: 40 }}>
                ...
                <p className="text-[5px] break-all md:text-xl sm:text-xs font-medium">
                  {battle.arttitle.length > 10 ? `${battle.arttitle.substring(0, 10)}...` : battle.arttitle} by {battle.artistId}
              </p>
                </div>
            </div>
        </a>
                        ...
                        <td className="px-4 py-2 text-xs  sm:text-2xl font-medium break-words  break-all text-black text-center special-winner" style={{ borderTopRightRadius: 20, borderBottomRightRadius: 20 }}>
                            {battle.battleTime ? formatDate(battle.battleTime) : "Date not available"}
                        </td>
                    </tr>
                </>
            ))}
