...
const CardHolder: React.FC<CardHolderProps> = ({
  ...
  return (
    <>
      <div
        className="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-[1.5rem] lg:gap-[1.5rem] md:gap-[1rem] gap-[0.5rem]"
        id="upcoming-grid"
      >
        {artData.map((art, index) => (
          <div key={index}>
            <Card
              art={art}
              myTicketsNew={myTicketsNew[art._id] || 0}
      ...
      {selectedArtId && overlayArt && !signToast && (
        <BuyRafflePopup
          overlayArt={overlayArt}
