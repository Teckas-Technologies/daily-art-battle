...
export const UpcomingGrid: React.FC<Props> = ({
  toggleUploadModal,
  uploadSuccess,
  campaignId,
  fontColor,
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (upcomingElement) {
      ...
      const elementPosition =
        upcomingElement.getBoundingClientRect().top + window.pageYOffset;
  ...
  const getArts = async () => {
    ...
    if (loading || page === null || page === 0 || sort === null || !campaignId)
      return;
    // if (page === 1) {
  ...
  return (
    <>
      <div
        className="upcoming-hero w-full h-auto bg-black md:pt-4 pt-0 pb-20"
        id="upcoming"
      >
        {/* Filters top section */}
        <div className="filters w-full flex flex-col md:flex-row md:justify-between md:items-center gap-4 md:px-[7.8rem] px-3 pb-5 pt-10">
          ...
          >
            <h2 className="spartan-light text-white md:text-md text-sm text-center md:text-left whitespace-nowrap overflow-hidden text-ellipsis">
              {sortLabel}
            </h2>
            ...
            {isOpen && (
              <div className="options absolute top-[100%] left-0 w-[150%] pt-4 rounded-3xl bg-black">
                <div
                  ...
                  onClick={() =>
                    handleSort({ value: "voteDsc", label: "Top Collected" })
                  }
                >
                ...
                <div
                  ...
                  onClick={() =>
                    handleSort({ value: "voteAsc", label: "Least Collected" })
                  }
                >
                  <h2 className="spartan-light text-sm">
                    Least Collected Arts
                  </h2>
                </div>
                <div
                ...
                <div
                  ...
                  onClick={() =>
                    handleSort({ value: "dateAsc", label: "Oldest First" })
                  }
                >
        ...
        <div
          className={`grid-view w-full flex justify-center md:px-[7rem] px-3 md:pt-5 ${
            loading ? "pb-1" : "pb-5"
          } bg-black`}
        >
          <CardHolder
            artData={upcomingArts}
            campaignId={campaignId}
            adminEmail={adminEmail}
            setRefresh={setRefresh}
            setSelectedArt={setSelectedArt}
            totalPage={totalPage}
            removeArtById={removeArtById}
            setHideFailed={setHideFailed}
          />
        </div>

        {empty && upcomingArts.length === 0 && (
          <div
            className={`empty w-full ${
              page === 1 || page === 0 || (page === null && "min-h-[25rem]")
            } min-h-[25rem] flex items-center justify-center gap-2 pb-20`}
          >
            <InlineSVG
              src="/icons/info.svg"
        ...
        {loading && (
          <div
            className={`upcoming-loader ${
              page === 1 || page === 0 || (page === null && "min-h-[25rem]")
            } min-h-[25rem] md:mb-0 mb-8 w-full justify-center`}
          >
            {" "}
            {/* md:h-[10rem] h-[5rem] */}
            <Loader md="10" sm="8" />
