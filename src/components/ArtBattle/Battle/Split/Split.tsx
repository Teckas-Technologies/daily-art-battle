  ...
  useEffect(() => {
    if (artA && campaignId) {
      const fetchArtAUsertickets = async () => {
        const tickets = await fetchArtUserRaffleCount(
          artA?.id as string,
          campaignId
        );
    ...
    <>
      ...
      <div className="flex md:flex-row flex-col w-full justify-center items-center mt-5">
        <div className="hero-left flex md:flex-row flex-col justify-end items-center md:w-[50%] w-full md:pr-[3.5rem] ">
          <div
            className="relative arta xl:w-[24.5rem] xl:h-[26rem] lg:w-[22.5rem] lg:h-[24rem] md:w-[20.5rem] md:h-[22rem] w-[17rem] h-[17rem] rounded-2xl cursor-pointer"
            onClick={toggleOverlayA}
          >
            <img
              src={artA.imageUrl}
              alt={artA.title}
            ...
            <div
              className={`absolute bottom-0 w-full flex items-center justify-end px-3 pb-4`}
            >
              <div
                className="like w-[2.5rem] h-[2.5rem] bg-white flex justify-center items-center rounded-full"
                onClick={() => handleImageClick(artA?.id)}
              >
                <InlineSVG
                  src={`/icons/${
                    artAMyTickets > 0 ? "uparrow.svg" : "heart.svg"
                  }`}
          ...
          <div className="mob-vote-btn w-full md:hidden flex justify-center">
            <div
              className={`vote-btn w-full flex justify-center md:justify-end mt-5 mb-10 pb-5`}
            >
              <div className="outside w-auto h-auto rounded-3xl">
                <div className="second-layer w-auto h-auto rounded-3xl">
                  <button
                    className={` battle-vote-btn px-6 py-3 rounded-3xl cursor-pointer`}
                  >
                    <h2 className="md:spartan-bold spartan-semibold font-bold text-xs md:text-sm">
                      Total Collects{" "}
                      <span className="text-green">{artATickets}</span>
                    </h2>
                  </button>
                </div>
              </div>
        ...
        <div className="hero-right flex md:flex-row flex-col justify-start items-center md:w-[50%] w-full md:pl-[3.5rem]">
          <div
            className="relative artb xl:w-[24.5rem] xl:h-[26rem] lg:w-[22.5rem] lg:h-[24rem] md:w-[20.5rem] md:h-[22rem] w-[17rem] h-[17rem] rounded-2xl cursor-pointer"
            onClick={toggleOverlayB}
          >
            <img
              src={artB.imageUrl}
              alt={artB.title}
            ...
            >
              <div
                className="like w-[2.5rem] h-[2.5rem] bg-white flex justify-center items-center rounded-full"
                onClick={() => handleImageClick(artB?.id)}
              >
                <InlineSVG
                  src={`/icons/${
                    artBMyTickets > 0 ? "uparrow.svg" : "heart.svg"
                  }`}
                  className="w-7 h-7 spartan-medium"
                />
              </div>
          ...
          <div className="mob-vote-btn w-full md:hidden flex justify-center">
            <div
              className={`vote-btn w-full flex justify-center md:justify-end mt-5 mb-5`}
            >
              <div className="outside w-auto h-auto rounded-3xl">
                <div className="second-layer w-auto h-auto rounded-3xl">
                  <button
                    className={` battle-vote-btn px-5 py-3 rounded-3xl cursor-pointer`}
                  >
                    <h2 className="md:spartan-bold spartan-semibold font-bold text-xs md:text-sm">
                      Total Collects{" "}
                      <span className="text-green">{artBTickets}</span>
                    </h2>
                  </button>
                </div>
              </div>
        ...
        {selectedArtId && overlayArt && (
          <BuyRafflePopup
            ...
            myTickets={
              overlayArt?._id === artA?.id ? artAMyTickets : artBMyTickets
            }
        ...
        {signToast && (
          <SignInPopup
            text="Sign In to Collect a Raffle Ticket!"
            infoMsg={infoMsg}
