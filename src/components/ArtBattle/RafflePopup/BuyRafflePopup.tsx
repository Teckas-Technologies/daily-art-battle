...
const socials = [
  {
    id: "facebook",
    label: "Facebook",
    icon: "/images/Facebook_New.png",
    link: `https://www.facebook.com/sharer/sharer.php?u=`,
  }, // quote=${encodeURIComponent("Collect and win!")}
  {
    id: "twitter",
    label: "Twitter",
    icon: "/images/Twitter_New.png",
    link: "https://twitter.com/intent/tweet?text=Collect%20and%20win!%0A&url=",
  },
  {
    id: "telegram",
    label: "Telegram",
    icon: "/images/Telegram_New.png",
    link: "https://t.me/share/url?text=Collect%20and%20win!%0A&url=",
  },
];

export const BuyRafflePopup: React.FC<Props> = ({
  overlayArt,
  setRefresh,
  campaignId,
  setSuccess,
  ...
  //   const othersCollects: number = totalCollects - myCollects;
  //   const newBuyCollect: number = tokenCount !== null ? tokenCount : 0;
  //   const totalMyCollects = myCollects + newBuyCollect;
  //   const updatedTotalCollects = othersCollects + totalMyCollects;

  //   const rawChance =
  //     updatedTotalCollects > 0
  //       ? (totalMyCollects / updatedTotalCollects) * 100
  //       : 0;

  //   const chanceOfWinning = rawChance >= 99.99 ? 99.99 : rawChance;
  ...
  const chanceCalculator = () => {
    ...
    if (updatedTotalCollects === 0) return "0.00";

    if (othersCollects === 0) return "100.00";

    const rawChance = (totalMyCollects / updatedTotalCollects) * 100;

    const minOthersChance = 0.01;
    const othersChance = Math.max(
      (othersCollects / updatedTotalCollects) * 100,
      minOthersChance * 100
    );

    const yourChance = 100 - othersChance;

    return yourChance.toFixed(2);
  };
    ...
    <div className="upcoming-popup-holder fixed top-0 z-50 w-full h-full flex items-center justify-center px-3">
      <div className="upcoming-popup lg:w-[43.5rem] md:w-[40.5rem] w-full h-auto max-h-[97vh] overflow-scroll lg:p-10 md:p-8  p-4 rounded-2xl bg-black">
        <div className="close-art w-full flex justify-end">
          <div
            className="close-icon md:w-[1.9rem] md:h-[1.9rem] w-[1.5rem] h-[1.5rem] flex items-center justify-center rounded-md cursor-pointer"
            onClick={handleClose}
          >
            <InlineSVG
              src="/icons/close.svg"
        ...
        <div className="artist-name flex items-center md:justify-start justify-center gap-3 py-1">
          <div className="artist flex items-center gap-2">
            ...
            />
            <h4 className="text-green spartan-medium md:text-md text-sm">
              Artist name
            </h4>
          </div>
          <div className="name">
            <h2 className="md:spartan-bold spartan-semibold text-sm md:text-lg">
              {overlayArt?.artistName}
            </h2>
          </div>
        </div>
        <div className="art-center w-full h-auto flex md:flex-row flex-col items-center justify-between lg:py-4 md:py-2 py-1">
          <div className="art-img md:w-[50%] w-full flex md:justify-start justify-center gap-7">
            <div className="img-outer p-[0.5rem] rounded-xl">
              <div className="relative img-holder lg:w-[15rem] lg:h-[15rem] md:w-[14rem] md:h-[14rem] w-[14rem] h-[14rem] rounded-xl">
                <img
                  src={overlayArt.colouredArt}
                  alt={overlayArt.arttitle}
                  className="w-full h-full rounded-xl"
                />

                <div
                  className={`absolute bottom-0 w-full flex items-center ${
                    !shareOverLay ? "justify-between" : "justify-end"
                  } px-3 pb-2`}
                >
                  {/* adminEmail === userMail */}
                  {!shareOverLay && (
                    <div
                      className="share w-[2.5rem] h-[2.5rem] bg-white flex justify-center items-center rounded-full cursor-pointer"
                  ...
                  <div className="like w-[2.5rem] h-[2.5rem] bg-white flex justify-center items-center rounded-full">
                    <InlineSVG
                      src={`/icons/${
                        myTickets > 0 ? "uparrow.svg" : "heart.svg"
                      }`}
                ...
                {shareOverLay && (
                  <div className="art-share-overlay absolute bottom-0 w-full h-full flex justify-center items-center">
                    <div
                      ref={overlayRef}
                      className="social-shares flex items-center justify-center gap-0"
                    >
                      {socials.map((social, index) => (
                        <a
                          key={index}
                          href={`${
                            social.link + BASE_URL + "?artId=" + overlayArt._id
                          }`}
                          className="social-share md:mx-0 mx-[-0.1rem] md:w-[3.6rem] md:h-[3.6rem] w-[3rem] h-[3rem] flex justify-center items-center rounded-full cursor-pointer"
                          target="_blank"
              ...
              <div className="art-info w-full flex justify-between items-center py-2 px-4">
                <div className="art-owner md:w-[10rem] w-[10rem]">
                  <h2 className="collect lg:text-md md:text-sm text-xs spartan-semibold md:w-[9rem] w-[5rem] truncate overflow-hidden whitespace-nowraps">
                    Collects
                  </h2>
                </div>
                <div className="upvotes">
                  <h2 className="text-green">
                    {overlayArt?.raffleTickets as number}
                  </h2>
                </div>
              </div>
            </div>
          ...
          <div className="art-info md:w-[50%] w-full">
            <h2 className="spartan-medium md:flex hidden md:text-lg text-md py-2 text-green md:text-left text-center des">
              Description
            </h2>
            <h6 className="saprtan-medium description-text md:text-md text-sm pb-1 md:pt-1 pt-2 md:text-left text-center leading-tight">
              {overlayArt.arttitle}
            </h6>
            ...
            <div className="upload-date md:flex hidden items-center md:justify-start justify-center md:gap-3 gap-6 md:py-2 py-1">
              <div className="date flex items-center gap-2">
                ...
                />
                <h4 className="text-green spartan-medium md:text-sm text-md">
                  Upload Date
                </h4>
              </div>
              <div className="name">
                <h2 className="spartan-semibold text-md">
                  {formatDate(
                    overlayArt.uploadedTime instanceof Date
                      ? overlayArt.uploadedTime.toISOString()
                      : overlayArt.uploadedTime
                  )}
                </h2>
              </div>
            </div>
            <div className="upvotes relative w-auto flex items-center md:justify-start justify-center gap-2 py-1 pt-3">
              <h2 className="spartan-semibold md:text-lg text-xl raffle-text">
                Raffle Tickets
              </h2>
              <InlineSVG
                src="/icons/info.svg"
              ...
              />
              {raffleInfo && (
                <div className="raffle-info absolute w-auto bottom-[95%] right-0 px-4 py-2 rounded-xl bg-red-500">
                  <h2 className="md:text-sm text-xs font-normal leading-tight">
                    Increase your chance to win
                    <br />a rare NFT by collecting
                    <br />
                    the Raffle Tickets!
                  </h2>
                </div>
            ...
            <div className="tickets flex items-center md:justify-start justify-center gap-2 py-2">
              ...
              <span className="text-green text-lg">
                {(myTickets as number) > 0 ? myTickets : "0"}
              </span>
            </div>
        ...
        <div className="art-bottom w-full h-auto">
          <div className="upvote-btn w-full flex flex-col items-center gap-10 md:pb-0 pb-5">
            <div className="enter-tickets w-full flex md:flex-row flex-col items-center justify-center gap-4">
              <h3 className="text-white spartan-semibold enter-text">
            ...
            <div className="buy-tickets relative flex flex-col items-center gap-3">
              <h3 className="coin-count spartan-semibold md:text-lg text-md flex items-center gap-2">
                {userDetails &&
                  userDetails?.user?.gfxCoin < raffleTicketAmount() && (
                    <InlineSVG
                      src="/icons/info.svg"
              ...
              {insufficientBal && (
                <div className="raffle-info absolute w-auto bottom-[100%] left-[-1rem] px-4 py-2 rounded-xl bg-red-500">
                  <h2 className="md:text-sm text-xs font-normal leading-tight">
                    Insufficient GFXvs Coins!
                  </h2>
                </div>
              )}
              <div
                className={`collect-btn ${
                  (tokenCount as number) < 1 && "dissabled"
                } flex items-center gap-2 justify-center py-[0.5rem] px-[3rem] rounded-[0.8rem]`}
                onClick={() => onVote(overlayArt._id)}
              >
                <InlineSVG
                  src="/icons/gfx-point.svg"
                  className="fill-current point-c w-8 h-8"
                />
                <h3 className="spartan-semibold text-white collect-text">
