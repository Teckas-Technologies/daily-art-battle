    ...
    if (req.method == "POST") {
      try {
        ...
        if (queryType == "daily") {
          ...
          const lastClaimed = new Date(checkin.lastClaimedDate).setUTCHours(
            0,
            0,
            0,
            0
          ...
          if (!checkin || checkin.streakDays < 7) {
            return res
              .status(400)
              .json({ message: "7-day streak not completed!" });
          }
          const today = new Date().setUTCHours(0, 0, 0, 0);
          const lastWeeklyClaimed = new Date(
            checkin.lastWeeklyClaimDate
          ).setUTCHours(0, 0, 0, 0);
          if (today === lastWeeklyClaimed) {
            return res
              .status(400)
              .json({ message: "Weekly reward already claimed today!" });
          }
