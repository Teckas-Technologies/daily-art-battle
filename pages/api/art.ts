       ...
       case 'POST':
        ...
        case 'GET':
          ...
          if (queryType === 'battles') {
            ...
            if(sort=='vote'){
              const battles = await findPreviousArtsByVotes(page,limit,campaignId);
              return res.status(200).json(battles);
            }else if(sort=='date'){
            const {pastBattles,totalDocuments,totalPages} = await findPreviousArts(page,limit,campaignId);
            return res.status(200).json({pastBattles,totalDocuments,totalPages});
            }else{
              const {battles,totalDocuments,totalPages} = await findPreviousArts(page,limit,campaignId);
